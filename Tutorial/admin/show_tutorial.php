...
<body style="background: #E0E0D1; margin:0; padding:0;">
	<div style="margin:0 auto; width:900px; position:relative; text-align:center;">
		...
		<div style="width:900px; background:#FCFCFA; margin-top:-2px; float:left;">
			...
			<?php $data=select("SELECT * FROM user_post ORDER BY id DESC");
                    foreach($data as $row) {
                ?>
            <p><?php echo $row->title; ?> | <a href="edit_tutorial.php?id=<?php echo $row->id ?>">Edit</a> | <a href="delete_tutorial.php?id=<?php echo $row->id ?>">Delete</a></p>
            <?php } ?>
